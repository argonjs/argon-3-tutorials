...
In addition, since you don't want anyone else to simply copy and that encrypted file, argon.js also requires you to store a set of url pattern's in the encrypted file, representing valid urls from which the key can be used. The url patterns should be in the *urls* field in the JSON, as an array of regular expression strings.

The format of the encrypted file, with some examples of possible url patterns, is:  
{% highlight json %}
{
